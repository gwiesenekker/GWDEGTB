/* Offline sample-count experiment on exact codec streams; does not modify EGTBs.
 * Build: cc -O3 -DNDEBUG -march=native -std=c11 -pthread
 *        benchmark_wdl_training.c -lzstd -o /tmp/bench-wdl-training
 * Run: /tmp/bench-wdl-training FILE.wdl (format 1 or 2).
 * Nested 1024/4096/16384 training sets, fixed 110 KiB dictionary, level 12.
 * All variants use the same 10000 disjoint held-out non-draw pages.
 */
...
int main(int argc,char **argv){
 ...
 unsigned counts[]={1024,4096,16384};
 for(unsigned d=0;d<3;d++){
  size_t cap=110*1024;
  unsigned char *dict=malloc(cap);CHECK(dict);double t=now();
  size_t n=ZDICT_trainFromBuffer(dict,cap,training,training_sizes,counts[d]);CHECK(!ZDICT_isError(n));
  printf("training,%u,%zu,%.6f\n",counts[d],n,now()-t);
  bench(pages+TRAIN,dict,n,12,0,dtm);
  free(dict);
 }
